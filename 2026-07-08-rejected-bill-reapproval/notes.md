...
**The demo starts from the approver's side too:** the **Awaiting Approval tab** is live, seeded with three first-round bills (two Boral, one A1 Asphalting) each sitting with a different approver and different workflow lengths. Open one to review it as the current approver — the detail page shows the workflow with a "Reviewing" marker and Approve/Reject actions. Rejecting opens the dialog (comment + optional approver suggestion), the bill flashes onto the Rejected tab, and from there the resubmit flow picks it up — the complete cycle in one sitting. Rejected/Awaiting tab counts stay in sync throughout.
