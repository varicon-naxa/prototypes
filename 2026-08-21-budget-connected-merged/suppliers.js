...
function spRender() {
  ...
  body.innerHTML = rows.length
    ? rows.slice(from, to).map(function (s) {
        ...
        return '<tr class="sp-row" data-name="' + spAttr(s.name) + '">' +
          ...
          '<td><span class="sp-acts">' +
            ...
            '<i class="fas fa-box-archive sp-archive" title="Archive"></i>' +
            '</span></td>' +
          '</tr>';
      }).join('')
  ...
  /* The row opens the supplier. The pencil still reads as the affordance but
     it is the same gesture now, so it carries no handler of its own — the
     checkbox and Archive are the only things that must not open the drawer. */
  body.onclick = function (ev) {
    var t = ev.target;
    if (t.closest('input[type=checkbox]') || t.closest('.sp-archive')) return;
    var tr = t.closest('.sp-row');
    if (tr) spEdit(tr.getAttribute('data-name'));
  };
