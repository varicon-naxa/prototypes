...
var VDATA = (function () {
  ...
  /* Employees are on the books and go through a timesheet. ABN contractors
     invoice against a purchase order — the base's model, and its budget splits
     a share of labour off to them. Either way the cost is labour: a contractor
     swinging a shovel is not a subcontract package.

     Which of the crew are contractors is fixed here rather than drawn at
     random, so the same person is the same kind of engagement everywhere. */
  var ABN_CREW = ['W-01', 'W-03'];

  function workers() {
    return DAYWORK_PEOPLE.map(function (p) {
      var abn = ABN_CREW.indexOf(p.id) >= 0;
      return {
        id: p.id, nm: p.name, role: p.role, rate: p.costRate, in: false,
        abn: abn,
        /* a contractor is engaged on an order, and invoices against it */
        po: abn ? 'PO-' + (2000 + (seed(p.id + 'abn') % 900)) : null
      };
    });
  }
  ...
  /* ── was an order raised, and what is the source? ───────────────────────
     A bill is not an order. Where a PO exists the PO is the source and the
     bill is one of the things that lands against it, next to the dockets. A
     bill stands as the source only when nothing was ordered first.

     Whether one was raised depends on what is being bought, which is a demo
     assumption held here rather than sprinkled about: a subcontractor is never
     engaged without one, supply usually is ordered, and sundries — a permit, a
     tank of fuel — often are not.

     Note what is NOT in the key: the cost state. The same arrangement tracked
     and billed is one order, not two. */
  function orderFor(arrangement, kind) {
    var h = seed(arrangement);
    var raised = kind === 'sub' ? true
               : kind === 'material' ? (h % 10) < 8
               : (h % 10) < 5;
    return raised
      ? { src: 'PO-' + (1000 + (h % 900)), srcType: 'PO', raised: true }
      : { src: 'BILL-' + (1000 + (h % 900)), srcType: 'Bill', raised: false };
  }

  /* What lands against the order on the day: a docket while it is tracked, a
     bill once it is paid. Different documents, so different references. */
  function eventFor(key, state, ord) {
    if (state !== 'actual') {
      return { ref: 'DKT-' + (1000 + (seed(key + 'dk') % 9000)), doc: 'Docket' };
    }
    /* When no order was raised the bill is the source, so the billed line is
       that same document — not a second bill number sitting under the first. */
    return ord && !ord.raised
      ? { ref: ord.src, doc: 'Bill' }
      : { ref: 'BILL-' + (4000 + (seed(key + 'bl') % 900)), doc: 'Bill' };
  }
  ...
  function orderTracker(kinds, iso) {
    ...
    l.days.forEach(function (d) {
      (l.byDate[d] || []).forEach(function (r) {
        ...
        if (r.detail.kind === 'labour' && !r.detail.abn) return;
        var dt = r.detail;
        var e = bySrc[dt.src] || (bySrc[dt.src] = {
          src: dt.src, srcType: dt.srcType, nm: dt.nm, sup: dt.sup,
          kind: dt.kind, raised: dt.raised,
          unit: dt.unit, rateUnit: dt.rateUnit || dt.unit,
          rate: dt.rate2 || dt.rate, cc: r.cc, code: dt.code,
          ccs: [], totalQty: 0, deliveries: []
        });
        e.totalQty += dt.qty || 0;
        if (e.ccs.indexOf(r.cc) < 0) e.ccs.push(r.cc);
        e.deliveries.push({
          date: d, docket: dt.docket, doc: dt.doc || 'Docket',
          qty: dt.qty || 0, cost: r.cost || 0,
    ...
    return Object.keys(bySrc).map(function (k) {
      ...
      /* An order is raised for more than has been taken from it — supply, a
         subcontract, and an ABN contractor's hours alike. Sundries are the
         exception: a permit is bought and it is done. */
      var ordered = (e.kind === 'misc' || !e.raised)
      ...
      if (e.kind === 'sub') {
        ...
        var asPc = function (x) {
          return { date: x.date, docket: x.docket, doc: x.doc, qty: pc(x.qty),
                   cost: x.cost, state: x.state, cc: x.cc, forWork: x.forWork };
        };
        return {
          id: e.src, src: e.src, srcType: e.srcType, nm: e.nm, sup: e.sup,
          kind: e.kind, raised: e.raised, unit: '%', rate: Math.round(value / 100),
      ...
      var allCost = e.deliveries.reduce(function (a, x) { return a + x.cost; }, 0);
      var blended = e.totalQty ? Math.round(allCost / e.totalQty) : e.rate;

      return {
        id: e.src, src: e.src, srcType: e.srcType, nm: e.nm, sup: e.sup,
        kind: e.kind, raised: e.raised,
        unit: e.unit, rateUnit: e.rateUnit || e.unit, rate: blended, code: e.code,
  ...
  function trackerLine(src, iso) {
    return orderTracker(['material', 'misc', 'sub', 'labour'], iso)
    ...
    if (cat === 'labour') {
      ...
      split(amount, weights).forEach(function (amt, i) {
        ...
        rows.push({
          ...
          detail: {
            kind: 'labour', worker: w, hrs: hrs, rate: w.rate,
            /* labour either way; the engagement is what differs */
            abn: w.abn, po: w.po,
            /* A contractor is engaged on an order, so the order is a real one:
               their timesheets draw down against it and their invoices land on
               it, the same way dockets and bills land on a supply PO. An
               employee has no order — they are on the books. */
            nm: w.abn ? w.nm + ' — ABN contract' : null,
            sup: w.nm, unit: 'hrs', rateUnit: 'hr', qty: hrs, rate2: w.rate,
            src: w.po, srcType: w.abn ? 'PO' : null, raised: true,
            docket: w.abn
              ? (state === 'actual' ? 'INV-' + (700 + (seed(key + w.id) % 90))
                                    : 'TS-' + (4000 + (seed(key + w.id) % 900)))
              : null,
            doc: state === 'actual' ? 'Invoice' : 'Timesheet',
            forWork: cc
          }
        });
      });
      ...
      split(amount, [3, 1.6].slice(0, nm2)).forEach(function (amt, i) {
        ...
        /* Keyed by material, supplier and unit — not by cost centre and not by
           cost state. One order of road base is drawn down by more than one
           cost centre, which is why a docket has to say which one it landed on;
           and the same order is both docketed and billed over its life. */
        var ord = orderFor(material + sup + unit, 'material');
        var ev = eventFor(key + material, state, ord);
        rows.push({
          ...
          detail: {
            ...
            src: ord.src, srcType: ord.srcType, raised: ord.raised,
            docket: ev.ref, doc: ev.doc,
      ...
      var subOrd = orderFor(sub + cc, 'sub');
      var subEv = eventFor(key + sub, state, subOrd);
      rows.push({
        ...
        detail: {
          ...
          src: subOrd.src, srcType: subOrd.srcType, raised: subOrd.raised,
          /* a claim while it is tracked; the bill once it is paid */
          docket: state === 'actual' ? subEv.ref
                                     : 'CLM-' + (100 + (seed(key + 'b') % 90)),
          doc: state === 'actual' ? 'Bill' : 'Claim',
      ...
      var mOrd = orderFor(m2.nm + sup2, 'misc');
      var mEv = eventFor(key + m2.nm, state, mOrd);
      rows.push({
        ...
        detail: {
          ...
          src: mOrd.src, srcType: mOrd.srcType, raised: mOrd.raised,
          docket: mEv.ref, doc: mEv.doc,
          forWork: cc
        }
      });
    }
  ...
  rows.forEach(function (r, i) {
    ...
    if (dt.kind === 'labour') {
      ...
      out.labour.push({
        ...
        cost: r.cost,
        /* how they are engaged, and for a contractor the order they sit on */
        abn: dt.abn, po: dt.po
      });
...
/* which order the panel is showing, so a change of view can re-render it */
var BU_OPEN = null;
...
function sdOpenBuildUp(src) {
  ...
  var sub = r.kind === 'sub';
  var drawn = sub ? 'Claimed' : r.kind === 'labour' ? 'Worked' : 'Delivered';
  var tileOrdered = !r.raised ? 'Supplied in total'
    : sub ? 'Subcontract' : r.kind === 'labour' ? 'Engaged' : 'Ordered';
  var tileLeft = sub ? 'Left to claim' : r.kind === 'labour' ? 'Left on order' : 'To deliver';
  ...
  document.getElementById('buBody').innerHTML =
    '<div class="bu-sum">' +
      '<div class="bu-tile"><span>' + tileOrdered + '</span><b>' + q(r.ordered) +
        ' ' + r.unit + '</b></div>' +
      '<div class="bu-tile"><span>' + drawn + '</span><b>' + q(r.delivered) +
        ' ' + r.unit + '</b></div>' +
      /* nothing was ordered, so there is nothing still to come */
      (r.raised
        ? '<div class="bu-tile"><span>' + tileLeft + '</span><b>' +
            q(r.remaining) + ' ' + r.unit + '</b></div>'
        : '') +
    '</div>' +
    '<p class="bu-sec">' +
      (!r.raised ? 'No order was raised' : sub ? 'The subcontract' : 'The order') +
    '</p>' +
    '<div class="bu-line"><span class="bu-date">Raised</span>' +
      '<span class="bu-ref">' + r.src + '</span>' +
      '<span>' + (!r.raised ? 'Billed direct \u2014 no purchase order'
        : sub ? 'Lump sum'
        : q(r.ordered) + ' ' + r.unit +
          (SDVIEW.rates ? ' @ $' + r.rate + '/' + (r.rateUnit || r.unit) : '')) + '</span>' +
      '<span class="bu-cost">' + m(sub ? r.value : r.ordered * r.rate) +
      '</span></div>' +
    '<p class="bu-sec" style="margin-top:20px">' +
      (r.raised ? drawn + ' against it, to ' : 'Billed, to ') + dmy(iso) + '</p>' +
    (landed.length
...
function sdCloseBuildUp() {
  ...
  /* the figures go with it — a closed panel holds nothing to read */
  document.getElementById('buBody').innerHTML = '';
  BU_OPEN = null;
}
